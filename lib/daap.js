...
daap.prototype.containers = function(sessionId, databaseId, callback, query) {
    query = (query) ?
            "&query='" + query + "'" : "";
...
daap.prototype.playContainer = function(sessionId, databaseId, containerId, callback) { //}, containerItemId) {
    // Problem with playspec is it takes focus on and opens the iTunes window
    var path = 'ctrl-int/1/playspec?'
               + "database-spec='dmap.itemid:" + databaseId + "'"
               + "&container-spec='dmap.itemid:" + containerId + "'"
               + '&session-id=' + sessionId;
