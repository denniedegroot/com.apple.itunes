...
// Does not work yet
daap.prototype.playAlbum = function(sessionId, albumId, callback){
    var href = url.parse('ctrl-int/1/cue?'
        + 'command=play&query=\'daap.songalbumid:' + albumId
        + '&sort=album'
...
daap.prototype.getAlbums = function(sessionId, databaseId, callback){
    var href = url.parse('databases/' + databaseId +
        '/groups?session-id=' + sessionId +
        '&meta=dmap.itemname,dmap.itemid,dmap.persistentid,daap.songartist' +
        '&type=music&group-type=albums' +
        '&sort=artist&include-sort-headers=1');
    var path = decodeURIComponent(href.path);
    http_get(this.host, this.port, path, callback);
}

// fetch albums example
// http://host:port/databases/36/groups?session-id=4800534&meta=dmap.itemname,dmap.itemid,dmap.persistentid,daap.songartist&type=music&group-type=albums&sort=artist&include-sort-headers=1
