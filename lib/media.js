...
/* Local functions */
function cache_artists(dbid, itemid, callback) {
    var items = [];
    var objects = {};
    ...
    daap.items(session.id, dbid, itemid, function (error, response) {
        ...
        if ('daap.playlistsongs' in response &&
            ...
            for (var i = 0, len = items.length; i < len; i++) {
                objects[items[i]['itemname']] = items[i];
            }

            if (config.cache_enabled) {
                for (i = 0, len = items.length; i < len; i++) {
                    if (!(cache.artists[items[i]['itemname']])) {
                        cache.artists[items[i]['itemname']] = items[i];
                    } else {
                        if (!(cache.artists[items[i]['itemname']].databaseplaylistsid.indexOf(itemid) > -1)) {
                            cache.artists[items[i]['itemname']].databaseplaylistsid.push(itemid);
                        }
                    }
                }
...
function cache_songs(dbid, itemid, callback) {
    var items = [];
    var objects = {};
    var config = Homey.manager('settings').get('settings');
    ...
    daap.items(session.id, dbid, itemid, function (error, response) {
        ...
        if ('daap.playlistsongs' in response &&
            ...
            if (config.cache_enabled) {
                for (i = 0, len = items.length; i < len; i++) {
                    if (!(cache.songs[items[i]['itemname']])) {
                        cache.songs[items[i]['itemname']] = items[i];
                    } else {
                        if (!(cache.songs[items[i]['itemname']].databaseplaylistsid.indexOf(itemid) > -1)) {
                            cache.songs[items[i]['itemname']].databaseplaylistsid.push(itemid);
                        }
                    }
                }
...
exports.get_artists = function (callback, args) {
    ...
    if (config.cache_enabled && cache.artistsid.indexOf(itemid) > -1) {
        ...
        cache_artists(dbid, itemid, function (error, items) {
            if (error) {
                return callback(null, []);
            }
...
exports.get_songs = function (callback, args) {
    ...
    if (config.cache_enabled && cache.songsid.indexOf(itemid) > -1) {
        ...
        cache_songs(dbid, itemid, function (error, items) {
            if (error) {
                return callback(null, []);
            }
...
exports.refresh_cache = function () {
    ...
    exports.get_libraries(function (error, items) {
        ...
        items.forEach(function (object) {
            cache_artists(object.databaseid, object.itemid, function () {});
        });
        items.forEach(function (object) {
            cache_songs(object.databaseid, object.itemid, function () {});
        });
    }, args);
};
