...
exports.action_set_repeat = function (callback, args) {
    setProperty('dacp.repeatstate', parseInt(args.repeat), callback);
};

exports.action_set_shuffle = function (callback, args) {
    setProperty('dacp.shufflestate', parseInt(args.shuffle), callback);
};

function setProperty(propertyName, value, callback) {
    var properties = {};

    properties[propertyName] = value;

    daap.setProperty(session.id, properties, function (error, response) {
        if (error) {
            return callback(error, false);
        }
